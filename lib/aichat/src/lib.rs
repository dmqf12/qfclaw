...
impl StreamProcessor {
    ...
    async fn process_chunk(&mut self, chunk: &[u8]) -> Result<Option<Value>, anyhow::Error> {
    ...
    async fn flush_reasoning(&self) {
        if !self.full_reasoning.is_empty() {
            let _ = SendMessage::new(&format!("Reasoning 🧠\n{}", self.full_reasoning))
                .fold()
                .send()
                .await;
        }
...
async fn chat(api_key: &str, base_url: &str, payload: &Value, reasoning_mode: &str) -> Result<serde_json::Value> {
    ...
    if !stream_out {
        ...
        while let Some(chunk) = stream.next().await {
            ...
            if let Some(final_value) = processor.process_chunk(&chunk).await? {
...
pub async fn main(user_input: &str) -> Result<()> {
    ...
    loop {
        let reply: Value = match chat(&api_key, &base_url, &payload, &reasoning_mode).await {
            Ok(resp) => resp,
            Err(e) => {
